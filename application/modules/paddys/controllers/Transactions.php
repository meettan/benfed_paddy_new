...
class Transactions extends MX_Controller {
    ...
    public function f_workorder_add() {
           ...
           if($_SERVER['REQUEST_METHOD'] == "POST"){
                ...
                $query = $this->db->get_where('td_work_order', array('pre_order_no =' => $this->input->post('pre_order_no')))->result();
                ...
                if (count($query) == 0)
                {
                    ...
                    // "pre_order_no" =>  'SCMF/'.$dist_sort_code.'/'.$kms_year_sort_code.'/Wo/',
                        "pre_order_no" =>  $dist_sort_code.'-'.$this->input->post('pre_order_no'),
                    ...
                    echo '<script>alert("Order no already Exist!")</script>';
                   // $this->session->set_flashdata('msg', 'Order no already Exist!');
                    redirect('paddys/transactions/f_workorder_add');
