...
class Add_new extends MX_Controller {
        ...
        if($_SERVER['REQUEST_METHOD'] == "POST") {
            ...
            if(!empty($_FILES['f_society_detail']['name']) && in_array($_FILES['f_society_detail']['type'],$csvMimes)){
                ...
                $csvFile = fopen($_FILES['f_society_detail']['tmp_name'], 'r');
                    ...
                    $data = array(
                        'sl_no'       =>  str_replace(')','',substr($line[2],strpos($line[2],'(')+1)),
                        'dist'         =>  trim($line[1]),
                        'block'        =>  '0'.$line[3],
                        'branch_id'    =>  trim($line[1]),
                        'society_code' =>  str_replace(')','',substr($line[2],strpos($line[2],'(')+1)),
                        'soc_name'     =>  $line[2],
                     //   'pan_no'       =>  $line[4],
                        'inchargename' =>  $line[5],
                        'ph_no'        =>  $line[6],
                       // 'agreementno'  =>  $line[7],
                        ...
                        $society_code = str_replace(')','',substr($line[2],strpos($line[2],'(')+1));
                            ...
                            $query = $this->db->get_where('md_society', array('society_code ='=> $society_code));
    ...
    public function f_mill() {
            ...
            //Retriving mill Details
               $select     =   array("sl_no","mill_name","mill_code","reg_no","ph_no","dist" );
	...
	public function f_milll() {
            //Retriving mill Details
            $select     =   array("sl_no",
            "mill_name","mill_code","block","reg_no","ph_no","dist" );
    ...
    public function f_mill_upload() {
        ...
        if($_SERVER['REQUEST_METHOD'] == "POST") {
            ...
            if(!empty($_FILES['f_mill_detail']['name']) && in_array($_FILES['f_mill_detail']['type'],$csvMimes)){
                ...
                $csvFile = fopen($_FILES['f_mill_detail']['tmp_name'], 'r');
                    ...
                    while(($line = fgetcsv($csvFile)) !== FALSE){
                            ...
                            $data = array(
                                'dist'         =>  trim($line[1]),
                                'block'        =>  '0'.trim($line[3]),
                                'mill_code'    =>  str_replace(')','',substr($line[2],strpos($line[2],'(')+1)),
                                'mill_name'    =>  trim($line[2]),
                                'branch_id'    =>  trim($line[1]),
                                'created_by'   =>  $this->session->userdata['loggedin']['user_name'],
                                'created_dt'   =>  date('Y-m-d')
                                );
             ...
             //sl_no,mill_name,replace(substr(mill_name,instr(mill_name,'(')+1),')','') FROM `md_mill`
            //For notification storing message
            $this->session->set_flashdata('msg', $j.' Record Successfully added!');
